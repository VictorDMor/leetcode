...
class Solution:
    def maxProbability(self, n: int, edges: List[List[int]], succProb: List[float], start: int, end: int) -> float:
        ...
        for i in range(len(edges)):
            if edges[i][0] not in graph.keys():
                graph[edges[i][0]] = [(edges[i][1], succProb[i])]
            else:
                graph[edges[i][0]].append((edges[i][1], succProb[i]))
            if edges[i][1] not in graph.keys():
        ...
        pqueue = []
        heapq.heappush(pqueue, (-1.0, start))
            ...
            try:
                ...
                for neighbor, edge_prob in graph[node]:
                    ...
                    if new_prob > probs[neighbor]:
                        ...
                        heapq.heappush(pqueue, (-probs[neighbor], neighbor))
            except KeyError: # there is no connection from the current node; start node is isolated
                break
        return 0