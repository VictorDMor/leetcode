...
class Solution:
    def findMinArrowShots(self, points):
        ...
        print(sorted_points)
        consecutive_points = [sorted_points[0]]
        for i in range(1, len(sorted_points)):
            if sorted_points[i][0] in range(sorted_points[i-1][0], sorted_points[i-1][1]):
                consecutive_points[-1][1] = sorted_points[i][1]
            else:
                consecutive_points.append(sorted_points[i])
